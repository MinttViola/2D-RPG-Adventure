...
public class TileService {
	...
	private void getImages(BufferedImage originalImg){
		for(int i = 1; i<tiles.length-1;i++){
			...
			tiles[i].img = ScaleAssetsService.ScaleImage(tileSizeForScale, tileSizeForScale, img);
		}
		int x=7*16;
		int y=0;
		BufferedImage img = originalImg.getSubimage(x, y, OriginalTileSize, OriginalTileSize);
		img = ImageUtil.TransperentImg(img);
		tiles[0].img = ScaleAssetsService.ScaleImage(tileSizeForScale, tileSizeForScale, img);
	}
}
